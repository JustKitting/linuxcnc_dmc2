...
A cuboid or another object with identifiable stable surfaces can exercise the known-reference registration path. It is not a prerequisite for the owner's irregular raw-stock workflow. After a relocation, add a new setup and estimate placement from retained measured geometry or identifiable machined features. Reuse design-space feature coordinates; regenerate their machine coordinates from the new placement.
...
### Owner clarification: oversized wood without an exact stock model

The owner specifies wood larger than the material required for the cuts, with an outline that is not guaranteed to match the modeled stock. The nominal CAD cuboid, its exact side planes and the finished dice surfaces are therefore not established reference surfaces on the unmachined blank. This correction changes the implementation priority to measured-stock reconstruction and placement within it; matching CAD planes is an optional path for features that actually correspond.

Source inspection found that `object_map/positional/fit.rs` already minimizes a Huber-weighted sphere-to-STL residual. `model_role` in the request is currently descriptive; choosing `reference-stock` does not change that objective or estimate unknown stock dimensions. Robust weights do not turn systematic extra stock, a taper or a missing corner into measurement noise. A low residual against an unsuitable shape is not the objective for this job.

Closed-form rotation and error minimization are not mutually exclusive: Horn's quaternion method minimizes a least-squares alignment error for corresponding measurements. Its use here would still require appropriate correspondences; a different optimizer alone cannot repair that missing premise. The proposed matched-plane initial-alignment modules had not been written when this correction arrived. [Horn, section 4](https://people.csail.mit.edu/bkph/papers/Absolute_Orientation_Scanned.pdf).

The revised implementation has separate responsibilities:

- **Estimate measured stock in machine coordinates.** Minimize measurement residuals while allowing stock dimensions, local edge directions and surface shape to vary. Retain the ordered rim and top/side samples, their original fine triggers, calibrated ball geometry and residuals. A rectangle or plane may summarize a supported local region; it must not force the complete blank into a quadrilateral, parallel-sided block or perpendicular-face model. Preserve coherent deviations and concavities instead of dropping them merely because they disagree with that summary. Any smoothing scale, noise model or complexity penalty needs explicit context and retained values.
- **Place the unchanged machining geometry within the stock estimate.** Search allowed translations and rotations for material coverage. Shortage of material is a different residual from mismatch between two supposed copies of a surface. Excess stock is expected. A candidate must report missing material and unmeasured coverage separately; a small average penalty must not hide a local deficit. Among supported placements, boundary clearance can guide the objective once machining allowances and setup constraints are supplied. No allowance, clearance or objective weight is selected by this clarification.
- **Register later setups using corresponding evidence.** Once a measured stock model or machined reference features exist, robust registration can relate a later scan to that retained geometry. Use only surviving corresponding features for that fit. Do not treat intentionally removed stock as an outlier or rescale the finished design to agree with new measurements.

For the dice job, the material that must remain during OP1 includes its connected envelopes and backing (`stage1_after`), not only the seven finished blanks. The nominal raw-stock dimensions stay CAD planning data until compared with the actual blank. The same already-applied OP2 flip convention remains in force. Top and single-height rim observations support only their measured regions; stock between or beyond those observations needs explicit modeling assumptions or further evidence. The present software does not yet reconstruct this stock or optimize containment; the active TODOs track that implementation.
