...
impl GeometryRole {
    fn description(self) -> (&'static str, &'static str, &'static str) {
        match self {
            Self::RequiredMaterial => (
                "operation-retained-material",
                "material/candidate-source-model-candidate.machine-mm.stl",
                "Unchanged material the operation must preserve, including backing and holding geometry, at its unreviewed candidate placement.",
            ),
            Self::MeasuredSurface => (
                "estimated-measured-stock-surface",
                "stock/stock-surface.machine-mm.stl",
                "Supported measured interpolation facets. Open regions and unmeasured volume remain unresolved; this is not a CAM stock solid.",
            ),
        }
    }
    fn json(self) -> String {
        ...
        format!(
            "{{\"role\":{},\"file\":{},\"interpretation\":{},\"frame\":\"LinuxCNC machine-mm\",\"mm_per_unit\":1,\"import_placement\":\"identity\",\"additional_model_transform\":\"none\"}}",
            quote(role),
            quote(file),
            quote(meaning)
        )
...
pub fn export(
    ...
    let scene = format!(
        ...
        quote(setup.as_str()),
        quote(&setup_label),
        quote(&candidate.frame),
        geometry.join(","),
        quote(candidate.design.as_str()),
        candidate.units,
        pose.json(),
        quote(mr.candidate.as_str()),
        quote(material_id.as_str()),
        quote(mesh_id.as_str()),
        quote(mr.surface.as_str()),
        quote(stock_state),
        quote(stock_message)
    );
