...
fn zeros_on_lattice_nodes_do_not_duplicate_coplanar_facets() {
    let r = settings();
    let grid = grid::Grid::new(&r).unwrap();
    let mesh = extract::run(&grid, &field(&grid, |p| p[2]), r.triangles, |_| Ok(Some(0))).unwrap();
...
fn missing_field_and_unsupported_facets_remain_holes() {
    ...
    let mesh = extract::run(&grid, &nodes, r.triangles, |_| Ok(Some(0))).unwrap();
    assert!(plane_area(&mesh) < 4.);
    assert!(!mesh.unresolved_cells.is_empty());
    let unsupported =
        extract::run(&grid, &field(&grid, |p| p[2]), r.triangles, |_| Ok(None)).unwrap();
...
fn explicit_budgets_reject_incomplete_requested_calculation() {
    ...
    assert!(format!(
        "{}",
        extract::run(&grid, &field(&grid, |p| p[2]), 1, |_| Ok(Some(0)))
            .err()
            .unwrap()
    )
    .contains("max_mesh_triangles"));
    assert!(extract::run(&grid, &[], r.triangles, |_| Ok(Some(0))).is_err());
}
...
fn field_requires_independent_checks_and_outward_full_facet_support() {
    ...
    assert!(result.facet_support(&facet, &sr, &r).unwrap().is_some());
    assert!(result
...
fn full_facet_cannot_bridge_disjoint_sample_support_disks() {
    ...
    let support = surface::support::Support::new(&samples, &stations[0], p, &sr).unwrap();
    let vertices = [[-1., -1., 0.], [1., -1., 0.], [1., 1., 0.]];
    assert!(vertices
        .iter()
        .all(|v| support.contains(*v, p, &sr).unwrap()));
    assert!(!support.covers_points(&vertices, p, &sr).unwrap());
}
