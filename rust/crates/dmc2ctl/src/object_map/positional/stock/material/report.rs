...
pub fn build(
    ...
    let mut csv_rows = String::from("region,source_triangle,machine_x_mm,machine_y_mm,machine_z_mm,cover_radius_mm,state,patch_capture,patch_sequence,checks,signed_outside_distance_mm,local_clearance_lower_mm,local_clearance_upper_mm,clearance_deficit_lower_mm,clearance_deficit_upper_mm\n");
    ...
    for (i, (sample, region)) in cover.iter().zip(regions).enumerate() {
        ...
        *counts.entry(state).or_insert(0usize) += 1;
        ...
        let detail = format!("{{\"region\":{i},\"source_triangle\":{source_tri},\"model_center_mm\":{},\"machine_center_mm\":{},\"cover_radius_mm\":{},\"state\":{},\"comparisons\":[{}]}}",json(sample.center),json(p),sample.radius,quote(state),comparisons.join(","));
        rows.push(detail);
        if region.state != State::LocallyInward {
            ...
            needs.push(format!("{{\"kind\":{},\"message\":{},\"region\":{i},\"source_triangle\":{source_tri},\"region_center_machine_mm\":{},\"region_radius_mm\":{},\"required_facet_outward_normal\":{},\"stock_normal_inferred_from_design\":false,\"machine_action_authorized\":false}}",quote(state),quote(message),json(p),sample.radius,json(n)));
        }
    }
