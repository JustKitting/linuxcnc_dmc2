...
use super::{Source, request::Request, select::Selection};
...
pub fn build(
    ...
    for (i, c) in selected.cells.iter().enumerate() {
        ...
        let fields = p
            ...
            .collect::<Vec<_>>()
            .join(",");
        cells.push(format!("{{\"cell\":{i},\"grid_index\":{:?},\"material_regions\":[{regions}],\"selected_priority\":{priority},\"already_searched_source_sequences\":{:?},\"nearest_original_request_xy_distance_mm\":{},\"state\":{},\"future_contact_role\":\"fit-or-observe-requires-new-capture\",\"acquisition\":{{\"entry\":{},\"entry_requirement\":{},\"entry_work_mm\":{:?},\"entry_machine_mm\":{:?},\"target_machine_mm\":{:?},\"request_contract_fields\":{{{fields}}},\"new_trigger\":null,\"entry_path_supplied\":false,\"machine_action_authorized\":false}}}}",c.key,c.original_sequences,c.nearest_request_mm,quote(if c.original_sequences.is_empty(){"new-top-column-proposal"}else{"existing-column-retained-not-selected"}),quote(p.entry.name()),quote(p.entry.recovery()),p.start,start_machine,target_machine));
    }
