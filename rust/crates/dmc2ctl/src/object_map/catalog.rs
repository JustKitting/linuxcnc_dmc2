...
impl Field {
    fn description(self) -> (&'static str, &'static str, &'static str) {
        match self {
            ...
            Self::Request => ("request", "Fit request file", "file"),
...
pub const OPERATIONS: &[Spec] = &[
    ...
    Spec { operation: Op::LoadRequest, name: "load-request", label: "Open fit request", description: "Open a request draft for editing. Save edits to a new file to preserve the original.", fields: &[Request] },
    ...
    Spec { operation: Op::ExportFit, name: "export-fit", label: "Export analysis", description: "Copy the candidate STL, point clouds, report and retained sources into a new directory.", fields: &[Object, Setup, Analysis, NewDirectory] },
...
impl Spec {
    pub fn result_kind(&self) -> &'static str {
        match self.operation {
            Op::Prepare | Op::LoadRequest => "request",
            _ => "json",
        }
    }
    fn json(&self) -> String {
