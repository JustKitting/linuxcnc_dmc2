...
pub const OPERATIONS: &[Spec] = &[
    ...
    Spec { operation: Op::PrepareSurface, name: "prepare-stock-surface", label: "Prepare 3D stock surfaces", description: "Select retained fine contacts and review the separate include/exclude table of original coarse-miss captures, including runs with no contacts. Fill calibration and neighborhood settings, keep independent checks and exclude unrelated objects from both selections. All included sources must share the declared physical frame and probe/error model. A single rim line cannot determine wall slope.", fields: &[Object, Setup] },
    ...
    Spec { operation: Op::PrepareMaterial, name: "prepare-material-check", label: "Prepare material check", description: "Select a machining footprint or 3D volume-placement candidate, then choose retained surfaces with an explicit no-contact model and comparison budgets in the request.", fields: &[Object, Setup, Analysis] },
    Spec { operation: Op::CheckMaterial, name: "check-material", label: "Check candidate material", description: "Compare unchanged required geometry to supported surface patches and finite retained no-contact sweeps. Retain original misses, conflicts, local shortages and unknown regions for placement and further measurements. This does not certify stock occupancy or issue probing commands.", fields: &[Object, Setup, NewAnalysis, Request] },
    Spec {
