    ...
    process::run(
        Command::new("bindgen")
            .args([
                HAL_HEADER,
                "--allowlist-function",
                "(hal_(init|exit|ready|malloc|pin_(bit|float|s32|u32)_new|export_funct)|rtapi_print_msg)",
                "--allowlist-type",
                "(hal_(bit|float|s32|u32|pin_dir)_t|msg_level_t)",
                "--allowlist-var",
                "(HAL_(IN|OUT|IO)|RTAPI_MSG_ERR)",
                "--use-core",
                "--no-layout-tests",
                "--output",
            ])
