...
from dmc2_validation import (
    ...
    validate as validate_offline_profile,
)
...
def validate_launch_files() -> list[str]:
    ...
    required_files = (
        ...
        ROOT / "live" / "hal" / "mesa_status_sources.hal",
        ROOT / "live" / "hal" / "status_latches.hal",
        ROOT / "live" / "hal" / "status_postgui.hal",
        ROOT / "native" / "bin" / "dmc2-serial-bridge",
        ROOT / "native" / "bin" / "dmc2-task-monitor",
        ...
        ROOT / "python" / "dmc2_axis" / "pendant_mode.py",
...
def start_persistent_service() -> int:
    ...
    command = [
        ...
        sys.executable,
        str(ROOT / "scripts" / "launch_live.py"),
        "--live",
    ]
    completed = subprocess.run(
        command,
